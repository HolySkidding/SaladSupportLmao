...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/HolySkidding/SaladSupportLmao/'..readfile('newvape/profiles/commit.txt')..'/'..select(1, path:gsub('newvape/', '')), true)
		end)
	...
	function whitelist:update(first)
		local suc = pcall(function()
			local _, subbed = pcall(function()
				return game:HttpGet('https://github.com/HolySkidding/whitelists')
			end)
			local commit = subbed:find('currentOid')
			commit = commit and subbed:sub(commit + 13, commit + 52) or nil
			commit = commit and #commit == 40 and commit or 'main'
			whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/HolySkidding/whitelists/'..commit..'/PlayerWhitelist.json', true)
		end)
...
run(function()
	...
	FloatMode = Fly:CreateDropdown({
		Name = 'Float Mode',
		List = {'CFrame', 'Velocity', 'Bounce', 'Floor', 'Jump', 'TP'},
		Function = function(val)
			...
			if callback then
				...
				if entitylib.isAlive then
					...
					for i,v in pairs(enums) do
						entitylib.character.Humanoid:SetStateEnabled(v, false)
					end
					entitylib.character.Humanoid:ChangeState(Enum.HumanoidStateType.Swimming)
					RunLoops:BindToHeartbeat("Swim", function()
						local rootvelo = entitylib.character.HumanoidRootPart.Velocity
						local moving = entitylib.character.Humanoid.MoveDirection ~= Vector3.zero
						entitylib.character.HumanoidRootPart.Velocity = ((moving or inputService:IsKeyDown(Enum.KeyCode.Space)) and Vector3.new(moving and rootvelo.X or 0, inputService:IsKeyDown(Enum.KeyCode.Space) and SwimVertical.Value or rootvelo.Y, moving and rootvelo.Z or 0) or Vector3.zero)
					end)
				end
			else
				GravityChangeTick = tick() + 0.1
				...
				if entitylib.isAlive then
					...
					table.remove(enums, table.find(enums, Enum.HumanoidStateType.None))
					for i,v in pairs(enums) do
						entitylib.character.Humanoid:SetStateEnabled(v, true)
					end
				end
			end
		end,
	...
	GamingChair = vape.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				chair.Size = Vector3.new(0.2, 0.18, 0.18)
				chair.CanCollide = false
				chair.Massless = true
				chair.Material = Enum.Material.SmoothPlastic
				chair.Parent = workspace
				local chairMesh = Instance.new('SpecialMesh')
				chairMesh.MeshType = Enum.MeshType.FileMesh
				chairMesh.MeshId = 'rbxassetid://12972961089'
				...
				local chairarms = Instance.new('Part') 
				chairarms.Color = chair.Color
				chairarms.Size = Vector3.new(0.014, 0.009, 0.012)
				chairarms.CFrame = chair.CFrame * CFrame.new(-0.1, -0.5, 0)
				chairarms.CanCollide = false
				chairarms.Parent = chair
				local chairarmsMesh = Instance.new('SpecialMesh') 
				chairarmsMesh.MeshType = Enum.MeshType.FileMesh
				chairarmsMesh.MeshId = 'rbxassetid://12972673898'
				chairarmsMesh.Parent = chairarms
				...
				local chairlegs = Instance.new('Part') 
				chairlegs.Color = chair.Color
				chairlegs.Name = 'Legs'
				chairlegs.Size = Vector3.new(0.18, 0.1, 0.18)
				chairlegs.CFrame = chair.CFrame * CFrame.new(0, -0.2, 0)
				chairlegs.CanCollide = false
				chairlegs.Parent = chair
				local chairlegsMesh = Instance.new('SpecialMesh') 
				chairlegsMesh.MeshType = Enum.MeshType.FileMesh
				chairlegsMesh.MeshId = 'rbxassetid://13003181606'
				chairlegsMesh.Parent = chairlegs
				local chairfan = Instance.new('Part') 
				chairfan.Color = chair.Color
				chairfan.Name = 'Fan'
				chairfan.Size = Vector3.new(0.1, 0.1, 0.1)
				chairfan.CFrame = chair.CFrame * CFrame.new(0, -0.4, 0)
				chairfan.CanCollide = false
				chairfan.Parent = chair
				local chairfanMesh = Instance.new('SpecialMesh') 
				chairfanMesh.MeshType = Enum.MeshType.FileMesh
				chairfanMesh.MeshId = 'rbxassetid://13004977292'
				chairfanMesh.Parent = chairfan
				local trails = {}
...
run(function()
	...
	ChatSpammer = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				spawn(function()
					while chatspammerhook and task.wait(ChatSpammerDelay.Value / 10) do
						if #ChatSpammerMessages.ObjectList > 0 then
							local message = ChatSpammerMessages.ObjectList[math.random(1, #ChatSpammerMessages.ObjectList)]
							game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(
								message,
								"All"
							)
							if not ChatSpammerHideWait.Enabled then
								waitnum = waitnum + 1
								game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(
									"Wait message: " .. tostring(waitnum),
									"All"
								)
							end
						end
	...
	ChatSpammerHideWait = ChatSpammer:CreateToggle({
		...
		Function = function(state)
			ChatSpammerHideWait.Enabled = state
		end,
		Default = true
	})
	...
	ChatSpammerMessages = ChatSpammer:CreateTextList({
		...
		Function = function(list)
			ChatSpammerMessages.ObjectList = list
		end
	})
end)
...
run(function()
	...
	local Freecam = vape.Categories.World:CreateModule({
		Name = "Freecam",
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function itemAdded(v, manual)
		if (not v:GetAttribute('Disguise')) and ((v:IsA('Accessory') and (not v:GetAttribute('InvItem')) and (not v:GetAttribute('ArmorSlot'))) or v:IsA('ShirtGraphic') or v:IsA('Shirt') or v:IsA('Pants') or v:IsA('BodyColors') or manual) then
			repeat
				task.wait()
				v.Parent = workspace.Parent
			until v.Parent == workspace.Parent
			v:ClearAllChildren()
			v:Destroy()
		end
	end
